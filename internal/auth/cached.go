...
// single key, so a hot key doesn't generate a write per request.
...
// fall through to the store; unknown keys are negatively cached.
...
// background goroutine uses context.Background because the request context
// may cancel before the write completes.
func (c *Cached) maybeTouch(keyID string) {
	c.touchedMu.Lock()
	last, ok := c.touchedAt[keyID]
	now := time.Now()
	if ok && now.Sub(last) < touchDebounce {
		c.touchedMu.Unlock()
		return
	}
	c.touchedAt[keyID] = now
	c.touchedMu.Unlock()

	go func() {
		if err := c.store.TouchLastUsedAt(context.Background(), keyID); err != nil {
